...
export default function VortexPage() {
  ...
  const aspectStyle = (ar: AspectRatio) =>
    ar === aspectRatio
      ? { border: `1px solid ${ACCENT}`, background: 'rgba(200,255,0,0.1)', color: ACCENT }
      : { border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED };

  const durationStyle = (d: Duration) =>
    d === duration
      ? { border: `1px solid ${ACCENT}`, background: 'rgba(200,255,0,0.1)', color: ACCENT }
      : { border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED };

  const styleTagStyle = (s: Style) =>
    s === style
      ? { border: `1px solid ${ACCENT}`, background: 'rgba(200,255,0,0.1)', color: ACCENT }
      : { border: `1px solid ${BORDER}`, background: 'transparent', color: MUTED };

  return (
    <div style={{ background: BG, minHeight: '100vh', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", color: '#fff' }}>
      <style>{`
        ...
        .vx-heading { font-family: 'Bebas Neue', Impact, sans-serif; letter-spacing: 0.06em; }
      ...
      <header style={{ borderBottom: `1px solid ${BORDER}`, padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: `${BG}ee`, backdropFilter: 'blur(12px)', zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontSize: '11px', color: MUTED, letterSpacing: '0.12em', textTransform: 'uppercase', marginLeft: '4px', paddingTop: '3px' }}>AI Video</span>
        </div>

        <button
          onClick={() => setShowApiModal(true)}
          style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '7px 14px', border: `1px solid ${apiKey ? 'rgba(200,255,0,0.3)' : BORDER}`, background: 'transparent', color: apiKey ? ACCENT : '#999', cursor: 'pointer', borderRadius: '8px', fontSize: '12px', fontFamily: 'inherit', letterSpacing: '0.04em' }}
        >
      ...
      <main style={{ maxWidth: '860px', margin: '0 auto', padding: '40px 20px 80px' }}>
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '28px', marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <textarea
              ...
              rows={4}
              style={{ width: '100%', background: BG, border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '14px', color: '#fff', fontSize: '15px', resize: 'vertical', outline: 'none', fontFamily: 'inherit', lineHeight: '1.6', boxSizing: 'border-box', transition: 'border-color 0.15s' }}
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {ASPECT_RATIOS.map(ar => (
                  <button key={ar} onClick={() => setAspectRatio(ar)}
                    style={{ flex: 1, padding: '10px 4px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontFamily: 'inherit', transition: 'all 0.15s', ...aspectStyle(ar) }}>
                    {ar}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {DURATIONS.map(d => (
                  <button key={d} onClick={() => setDuration(d)}
                    style={{ flex: 1, padding: '10px 4px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontFamily: 'inherit', transition: 'all 0.15s', ...durationStyle(d) }}>
                    {d}s
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {STYLES.map(s => (
                <button key={s} onClick={() => setStyle(s)}
                  style={{ padding: '8px 18px', borderRadius: '100px', cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit', transition: 'all 0.15s', ...styleTagStyle(s) }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '18px',
              background: !prompt.trim() || isGenerating ? '#1e1e22' : ACCENT,
              color: !prompt.trim() || isGenerating ? '#444' : BG,
              border: 'none',
              borderRadius: '10px',
              fontSize: '20px',
        ...
        {isGenerating && (
          <div style={{ background: CARD, border: `1px solid rgba(200,255,0,0.15)`, borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
            ...
            <p style={{ color: '#444', fontSize: '11px', marginTop: '10px', letterSpacing: '0.04em' }}>
              This typically takes 1–3 minutes. Grab a coffee ☕
            </p>
          </div>
        )}
          ...
          <div style={{ background: CARD, border: `1px solid rgba(200,255,0,0.2)`, borderRadius: '16px', padding: '24px', marginBottom: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <a
                ...
                rel="noreferrer"
                download
                style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: ACCENT, color: BG, borderRadius: '8px', fontSize: '13px', fontWeight: '600', textDecoration: 'none', fontFamily: 'inherit' }}
              >
            ...
            <video
              ...
              style={{
                ...
                background: '#000',
                display: 'block',
                maxHeight: aspectRatio === '9:16' ? '500px' : '480px',
                margin: '0 auto',
        ...
        {history.length > 0 && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button
                onClick={() => { setHistory([]); localStorage.removeItem('vortex_history'); }}
                style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#444', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '12px', fontFamily: 'inherit' }}
              >
      ...
      {showApiModal && (
        ...
        >
          <div
            ...
            style={{ background: '#15151a', border: `1px solid rgba(255,255,255,0.1)`, borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '440px' }}
          >
            ...
            <p style={{ color: MUTED, fontSize: '13px', marginBottom: '20px', lineHeight: '1.6' }}>
              ...
              Your key is stored only in your browser&apos;s localStorage and never sent to any third party.
            </p>
            <input
              type="password"
              value={apiKeyInput}
              ...
              style={{ width: '100%', padding: '12px 14px', background: BG, border: `1px solid ${BORDER}`, borderRadius: '8px', color: '#fff', fontSize: '13px', fontFamily: 'monospace', outline: 'none', boxSizing: 'border-box', marginBottom: '14px', transition: 'border-color 0.15s' }}
            ...
            {apiKey && (
              <button
                onClick={() => { localStorage.removeItem('vortex_api_key'); setApiKey(''); setApiKeyInput(''); }}
                style={{ display: 'block', width: '100%', padding: '10px', background: 'transparent', border: `1px solid rgba(255,70,70,0.2)`, borderRadius: '8px', color: '#ff6666', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', marginBottom: '10px' }}
              >
                Remove saved key
              </button>
            )}
            <button
              onClick={saveKey}
              style={{ display: 'block', width: '100%', padding: '14px', background: ACCENT, color: BG, border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Save API Key
            </button>
          </div>
